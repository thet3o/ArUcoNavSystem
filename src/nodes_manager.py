...
nodes_list_layout = [
    ...
    [
        pyg.Listbox(
            values=[node.id for node in nodes], enable_events=True, key='-NODES LIST-', size=(25, 50)
        )
    ]
]
...
update_add_node = [
    [
        pyg.Text('Add/Update Node'),
    ],
    [
        pyg.In(size=(25,1), enable_events=True, key="ID FIELD",),
    ],
    [
    ...
    [
        pyg.Listbox(
            values=[], enable_events=True, key='WEIGHT NODE LIST', size=(20, 20), auto_size_text=True
        )
    ],
]
...
window = pyg.Window('Nodes Manager', layout=layout)
